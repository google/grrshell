...
_MOCK_LINUX_CLIENT = client.Client(
    data=text_format.Parse(open(_MOCK_LINUX_CLIENT_PROTO_FILE, 'rb').read().decode('utf-8'), client_pb2.ApiClient()),
    context=True)
_MOCK_WINDOWS_CLIENT_PROTO_FILE = 'grrshell/tests/testdata/mock_client_windows.textproto'
_MOCK_WINDOWS_CLIENT = client.Client(
...
_MOCK_APIFLOW_CFF_HASH_RUNNING = flow.Flow(
    data=text_format.Parse(open(_MOCK_APIFLOW_CFF_HASH_RUNNING_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlow()),
    context=mock.MagicMock())
...
_MOCK_APIFLOW_CFF_DOWNLOAD_RUNNING = flow.Flow(
    data=text_format.Parse(open(_MOCK_APIFLOW_CFF_DOWNLOAD_RUNNING_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlow()),
    context=mock.MagicMock())
...
_MOCK_APIFLOW_CFF_DOWNLOAD_TERMINATED = flow.Flow(
    data=text_format.Parse(open(_MOCK_APIFLOW_CFF_DOWNLOAD_TERMINATED_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlow()),
    context=mock.MagicMock())
...
_MOCK_APIFLOW_CFF_DOWNLOAD_ERROR = flow.Flow(
    data=text_format.Parse(open(_MOCK_APIFLOW_CFF_DOWNLOAD_ERROR_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlow()),
    context=mock.MagicMock())
...
_MOCK_APIFLOW_INTERROGATE_RUNNING = flow.Flow(
    data=text_format.Parse(open(_MOCK_APIFLOW_INTERROGATE_RUNNING_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlow()),
    context=mock.MagicMock())
...
_MOCK_HASH_LINUX_ENTRY = flow.FlowResult(
    data=text_format.Parse(open(_MOCK_HASH_LINUX_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlowResult()))
...
_MOCK_HASH_WINDOWS_ENTRY = flow.FlowResult(
    data=text_format.Parse(open(_MOCK_HASH_WINDOWS_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlowResult()))
...
_MOCK_HASH_WINDOWS_WITH_ADS_ENTRY = flow.FlowResult(
    data=text_format.Parse(open(_MOCK_HASH_WINDOWS_WITH_ADS_PROTO_FILE, 'rb').read().decode('utf-8'), flow_pb2.ApiFlowResult()))
...
_MOCK_ZIP_LINUX_CLIENTFILEFINDER_DATA = open(_MOCK_ZIP_LINUX_CLIENTFILEFINDER_FILE, 'rb').read()
_MOCK_ZIP_WINDOWS_CLIENTFILEFINDER_FILE = 'grrshell/tests/testdata/file_collect_windows.zip'
_MOCK_ZIP_WINDOWS_CLIENTFILEFINDER_DATA = open(_MOCK_ZIP_WINDOWS_CLIENTFILEFINDER_FILE, 'rb').read()
_MOCK_ZIP_DARWIN_ARTIFACTCOLLECTORFLOW_FILE = 'grrshell/tests/testdata/artifact_collect_darwin.zip'
_MOCK_ZIP_DARWIN_ARTIFACTCOLLECTORFLOW_DATA = open(_MOCK_ZIP_DARWIN_ARTIFACTCOLLECTORFLOW_FILE, 'rb').read()
_MOCK_ZIP_LINUX_ARTIFACTCOLLECTORFLOW_FILE = 'grrshell/tests/testdata/artifact_collect_linux.zip'
_MOCK_ZIP_LINUX_ARTIFACTCOLLECTORFLOW_DATA = open(_MOCK_ZIP_LINUX_ARTIFACTCOLLECTORFLOW_FILE, 'rb').read()
_MOCK_ZIP_WINDOWS_ARTIFACTCOLLECTORFLOW_FILE = 'grrshell/tests/testdata/artifact_collect_windows.zip'
_MOCK_ZIP_WINDOWS_ARTIFACTCOLLECTORFLOW_DATA = open(_MOCK_ZIP_WINDOWS_ARTIFACTCOLLECTORFLOW_FILE, 'rb').read()
_MOCK_ZIP_WINDOWS_GETFILE_ADS_FILE = 'grrshell/tests/testdata/getfile_ads.zip'
_MOCK_ZIP_WINDOWS_GETFILE_ADS_DATA = open(_MOCK_ZIP_WINDOWS_GETFILE_ADS_FILE, 'rb').read()
_MOCK_ZIP_WINDOWS_GETFILE_ADS_EMPTY_FILE = 'grrshell/tests/testdata/getfile_ads_empty.zip'
_MOCK_ZIP_WINDOWS_GETFILE_ADS_EMPTY_DATA = open(_MOCK_ZIP_WINDOWS_GETFILE_ADS_EMPTY_FILE, 'rb').read()
...
def _BuildMockArtifactDescriptors() -> list[artifact.Artifact]:
  ...
  for proto_file in artifactdescriptor_proto_files:
    to_return.append(artifact.Artifact(
        data=text_format.Parse(open(proto_file, 'rb').read().decode('utf-8'), artifact_pb2.ArtifactDescriptor()),
        context=mock.MagicMock()))
  return to_return
...
class GrrShellClientLinuxTest(parameterized.TestCase):
  ...
  def test_NoApproval(self, mock_InitHttp):
    ...
    mock_grr_api.Client.return_value.VerifyAccess.side_effect = grr_errors.AccessForbiddenError('No approval')

    with self.assertRaisesRegex(errors.NoGRRApprovalError, 'No approval for client access to C.0000000000000001'):
      self.client = grr_shell_client.GRRShellClient(
          _TEST_GRR_URL, _TEST_GRR_USER, _TEST_GRR_PASS, _TEST_CLIENT_FQDN, _MAX_FILE_SIZE_1GB)
  ...
  def test_InvalidClient(self, mock_InitHttp):
    ...
    with self.assertRaisesRegex(errors.ClientNotFoundError, f'0 potential clients found with search {_TEST_CLIENT_FQDN}'):
      self.client = grr_shell_client.GRRShellClient(
          _TEST_GRR_URL, _TEST_GRR_USER, _TEST_GRR_PASS, _TEST_CLIENT_FQDN, _MAX_FILE_SIZE_1GB)
  ...
  def test_GetLastSeenTime(self):
    ...
    with mock.patch.object(self.client._grr_client, 'Get', return_value=_MOCK_LINUX_CLIENT):
      result = self.client.GetLastSeenTime()
      self.assertEqual(result, datetime.datetime(2023, 5, 24, 1, 24, 23, 783055,
  ...
  def test_GetLastTimelineCorrect(self, mock_get):
    ...
    with (mock.patch.object(self.client._grr_client, 'ListFlows', return_value=_MOCK_APIFLOW_LISTFLOWS),
          mock.patch.object(flow.Flow, 'ListResults', autospec=True) as mock_list_results):
  ...
  def test_CollectTimelineNew(self):
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      mock_create_flow.assert_called_once_with(name='TimelineFlow', args=mock_create_flow_args.return_value)
  ...
  def test_CollectTimelinePath(self):
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs', return_value=mock.Mock()) as mock_create_flow_args,
          mock.patch.object(self.client._grr_client, 'CreateFlow', return_value=mock.Mock()) as mock_create_flow):
      mock_create_flow.return_value.ListResults.return_value = []
      self.client.CollectTimeline(path='/home/testuser/')

      mock_create_flow_args.assert_called_once_with('TimelineFlow')
      self.assertEqual(mock_create_flow_args.return_value.root, b'/home/testuser/')
      mock_create_flow.assert_called_once_with(name='TimelineFlow', args=mock_create_flow_args.return_value)
      mock_create_flow.return_value.WaitUntilDone.assert_called_once()
      mock_create_flow.return_value.GetCollectedTimelineBody.assert_called_once()
  ...
  def test_FileInfo(self, mock_list_results, mock_wait_until_done):
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      self.assertEqual(mock_create_flow_args.return_value.action.hash.max_size, _MAX_FILE_SIZE_1GB)
      mock_create_flow_args.return_value.paths.append.assert_called_once_with('/remote/path')
      self.assertEqual(mock_create_flow_args.return_value.action.action_type, flows_pb2.FileFinderAction.HASH)
      mock_create_flow.assert_called_once_with(name='ClientFileFinder', args=mock_create_flow_args.return_value)
  ...
  def test_CollectFiles(self,
    ...
    mock_list_results.return_value = [_MOCK_HASH_LINUX_ENTRY]
    mock_get_files_archive.return_value = [_MOCK_ZIP_LINUX_CLIENTFILEFINDER_DATA]
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      mock_create_flow_args.assert_called_once_with('ClientFileFinder')
      self.assertEqual(mock_create_flow_args.return_value.action.download.max_size, _MAX_FILE_SIZE_1GB)
      self.assertEqual(mock_create_flow_args.return_value.pathtype, jobs_pb2.PathSpec.OS)
      mock_create_flow_args.return_value.paths.append.assert_called_once_with('/remote/path')
      mock_create_flow.assert_called_once_with(name='ClientFileFinder', args=mock_create_flow_args.return_value)
      mock_wait_until_done.assert_called_once()
  ...
  def test_CollectArtifact(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      mock_create_flow_args.return_value.artifact_list.append.assert_called_once_with('artifact_name')
  ...
  def test_CollectFilesInBackground(self,
    ...
    mock_get.side_effect = [_MOCK_APIFLOW_CFF_DOWNLOAD_RUNNING, _MOCK_APIFLOW_CFF_DOWNLOAD_TERMINATED]
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      actual_states = self.client.GetBackgroundFlowsState()
      self.assertEqual(actual_states, '\tCLIENTFILEFINDERRUNNINGFLOWID ClientFileFinder DOWNLOAD /remote/path RUNNING')

      running, total = self.client.GetRunningFlowCount()
      self.assertEqual(running, 1)
      self.assertEqual(total, 1)
      actual_states = self.client.GetBackgroundFlowsState()
      self.assertEqual(actual_states, '\tCLIENTFILEFINDERTERMINATEDFLOWID ClientFileFinder DOWNLOAD /remote/path DOWNLOADING')

      running, total = self.client.GetRunningFlowCount()
      self.assertEqual(running, 0)
      self.assertEqual(total, 1)
      actual_states = self.client.GetBackgroundFlowsState()
      self.assertEqual(actual_states, '\tCLIENTFILEFINDERTERMINATEDFLOWID ClientFileFinder DOWNLOAD /remote/path COMPLETE')
  ...
  def test_SetMaxFileSize(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      self.assertIn('download', mock_create_flow_args.return_value.action.__dict__['_mock_children'])
      self.assertEqual(mock_create_flow_args.return_value.action.download.max_size, 1024)
  ...
  def test_DefaultMaxFileSize(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      self.assertNotIn('download', mock_create_flow_args.return_value.action.__dict__['_mock_children'])
...
class GrrShellClientWindowsTest(parameterized.TestCase):
  ...
  def test_FileInfo_NoADS(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
          mock.patch.object(self.client._grr_client, 'CreateFlow') as mock_create_flow):
      mock_create_flow.side_effect = [_MOCK_APIFLOW_CFF_DOWNLOAD_RUNNING, _MOCK_APIFLOW_GETFILE_RUNNING]

      result = self.client.FileInfo('/C:/Users/username/Downloads/Firefox Installer.exe', collect_ads=True)
      ...
      mock_create_flow.assert_has_calls([
          mock.call(name='ClientFileFinder', args=mock_create_flow_args.return_value),
          mock.call(name='GetFile', args=flows_pb2.GetFileArgs(
  ...
  def test_FileInfo_wildcard(self, mock_list_results, mock_wait_until_done):
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      mock_create_flow_args.assert_called_once_with('ClientFileFinder')
      self.assertEqual(mock_create_flow_args.return_value.action.hash.max_size, _MAX_FILE_SIZE_1GB)
      mock_create_flow_args.return_value.paths.append.assert_called_once_with('C:/Users/username/Downloads/Firefox*')
      self.assertEqual(mock_create_flow_args.return_value.action.action_type, flows_pb2.FileFinderAction.HASH)
      mock_create_flow.assert_called_once_with(name='ClientFileFinder', args=mock_create_flow_args.return_value)
      mock_wait_until_done.assert_called_once()
  ...
  def test_FileInfo_WithADS(self,
    ...
    mock_gf_get_files_archive.return_value = [_MOCK_ZIP_WINDOWS_GETFILE_ADS_DATA]
  ...
  def test_FileInfo_WithEmptyADS(self,
    ...
    mock_ff_list_results.return_value = [_MOCK_HASH_WINDOWS_ENTRY]
    mock_gf_list_results.return_value = [_MOCK_HASH_WINDOWS_WITH_ADS_ENTRY]
    mock_gf_get_files_archive.return_value = [_MOCK_ZIP_WINDOWS_GETFILE_ADS_EMPTY_DATA]
  ...
  def test_FileInfo_ClientFileFinderFailure(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs'),
      ...
      mock_ff_wait_until_done.side_effect = grr_errors.FlowFailedError('test error')

      with io.StringIO() as buf, contextlib.redirect_stdout(buf):
        self.client.FileInfo('/C:/Users/username/Downloads/Firefox Installer.exe')
        self.assertIn('HASH Flow collection CLIENTFILEFINDERRUNNINGFLOWID failed: test error', buf.getvalue())
  ...
  def test_FileInfo_ADSFailure(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs'),
          mock.patch.object(self.client._grr_client, 'CreateFlow') as mock_create_flow):
      mock_create_flow.side_effect = [_MOCK_APIFLOW_CFF_DOWNLOAD_RUNNING, _MOCK_APIFLOW_GETFILE_RUNNING]
      mock_gf_wait_until_done.side_effect = grr_errors.FlowFailedError('test error')

      with io.StringIO() as buf, contextlib.redirect_stdout(buf):
        result = self.client.FileInfo('/C:/Users/username/Downloads/Firefox Installer.exe', collect_ads=True)
        self.assertIn('ADS Flow collection GETFILERUNNINGFLOWID failed: test error', buf.getvalue())
        mock_ff_wait_until_done.assert_called_once()
        self.assertEqual(result, _EXPECTED_HASH_WINDOWS_RESULT)
  ...
  def test_CollectFiles(self,
    ...
    mock_get.side_effect = [_MOCK_APIFLOW_CFF_DOWNLOAD_TERMINATED]
    mock_list_results.return_value = [_MOCK_HASH_WINDOWS_ENTRY]
    mock_get_files_archive.return_value = [_MOCK_ZIP_WINDOWS_CLIENTFILEFINDER_DATA]
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      local_path = os.path.join(self.create_tempdir(), 'local_path')
      self.client.CollectFiles('/C:/Users/username/Downloads/Firefox Installer.exe', local_path)

      mock_create_flow_args.assert_called_once_with('ClientFileFinder')
      self.assertEqual(mock_create_flow_args.return_value.pathtype, jobs_pb2.PathSpec.NTFS)
      self.assertTrue(mock_create_flow_args.return_value.use_raw_filesystem_access)
  ...
  def test_Resume_GetFile(self,
    ...
    self.mock_grr_api.Client.return_value.Flow.return_value.Get.return_value = _MOCK_APIFLOW_GETFILE_RUNNING
    ...
    mock_wait_until_done.assert_called_once()
    self.assertEqual(results, _EXPECTED_ZONE_IDENTIFIER_RESULT)
  ...
  def test_Resume_ClientFileFinder_Hash(self,
    ...
    self.mock_grr_api.Client.return_value.Flow.return_value.Get.return_value = _MOCK_APIFLOW_CFF_HASH_RUNNING
    mock_list_results.return_value = [_MOCK_HASH_WINDOWS_ENTRY]

    results = '\n'.join(self.client.ResumeFlow('CLIENTFILEFINDERHASHRUNNINGFLOWID'))
    mock_wait_until_done.assert_called_once()
  ...
  def test_Resume_InvalidFlow(self):
    ...
    self.mock_grr_api.Client.return_value.Flow.return_value.Get.return_value = _MOCK_APIFLOW_TIMELINE_RUNNING

    with self.assertRaisesRegex(errors.NotResumeableFlowTypeError,
                                'Flow TIMELINEFLOWID is of type TimelineFlow, not supported for resumption.'):
  ...
  def test_CollectTimelinePath(self, in_path, expected_path):
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs', return_value=mock.Mock()) as mock_create_flow_args,
      ...
      mock_create_flow_args.assert_called_once_with('TimelineFlow')
      self.assertEqual(mock_create_flow_args.return_value.root, expected_path)
      mock_create_flow.assert_called_once_with(name='TimelineFlow', args=mock_create_flow_args.return_value)
      mock_create_flow.return_value.WaitUntilDone.assert_called_once()
      mock_create_flow.return_value.GetCollectedTimelineBody.assert_called_once()
...
class GrrShellClientDarwinTest(parameterized.TestCase):
  ...
  def test_CollectFiles(self,
    ...
    mock_get.side_effect = [_MOCK_APIFLOW_CFF_DOWNLOAD_TERMINATED]
    mock_list_results.return_value = [_MOCK_HASH_LINUX_ENTRY]
    mock_get_files_archive.return_value = [_MOCK_ZIP_DARWIN_CLIENTFILEFINDER_DATA]
  ...
  def test_CollectArtifact(self,
    ...
    with (mock.patch.object(self.client._grr_stubby.types, 'CreateFlowArgs') as mock_create_flow_args,
      ...
      self.assertTrue(os.path.exists(os.path.join(local_path, 'Users', 'username', 'file')))
