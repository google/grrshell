...
_TIMELINE_THRESHOLD_HELP = (
    'How old before a preexisting timeline is considered too old. Format '
    'example: `7d` (7 days); `12h` (12 hours)')
# go/keep-sorted end
...
class Main:
  ...
  def DefineFlags(cls):
    ...
    flags.DEFINE_string(
        name='timeline-threshold', default='12h', required=False,
  ...
  def _ParseArgs(self, argv: Sequence[str]) -> bool | str:
    ...
    try:
      seconds = dehumanize.DurationSeconds(
    ...
    self._timeline_threshold = datetime.timedelta(seconds=seconds)
    logger.debug('timeline-threshold %s converted to %s seconds',
                 flags.FLAGS['timeline-threshold'].value,
                 seconds)
